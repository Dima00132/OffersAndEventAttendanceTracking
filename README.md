...
Маил-сервер — параметры соединения по SMTP в Unisender Go:
smtp.go1.unisender.ru, smtp.go2.unisender .ru — указание домена;
порт — 25*, 465 или 587;
включение зашифрованного соединения.

Последняя строка — заполнение данных из Unisender Go, которые включают доменную почту, имеющуюся у пользователя (после приобретения домена и создания соответствующей почты). Также требуется указать аккаунт пользователя в Unisender Go и пароль — API-ключ пользователя или project_api_key.
