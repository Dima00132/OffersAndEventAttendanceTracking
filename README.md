...
3. Последняя строка — заполнение данных из [Unisender Go](https://go1.unisender.ru/ru), которые включают доменную почту, имеющуюся у пользователя (после приобретения домена и создания соответствующей почты). Также требуется указать аккаунт пользователя в [Unisender Go](https://go1.unisender.ru/ru) и пароль — API-ключ пользователя или project_api_key.
